...
__global__ void calThreadPerColumn(int* output, int graph_size, int numThreads, int k)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void floyd_warshall_gpu(const int *graph, int graph_size, int *output) {
	...
	/* Needed for calThreadPerColumn
	int t = 64;
	int b = prop.multiProcessorCount * (prop.maxThreadsPerMultiProcessor / NThreads);
	*/
